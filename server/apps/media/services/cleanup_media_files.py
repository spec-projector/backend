...
class CleanupMediaFilesService(ICleanupMediaFilesService):
    ...
    def cleanup_media_files(self, instance: models.Model) -> None:
        """Cleanup media files."""
        self._cleanup_files(instance)
        self._cleanup_images(instance)

    def _cleanup_files(self, instance: models.Model) -> None:
        for file_field in self._get_media_fields(instance, File):
            media_instance = getattr(instance, file_field.name)
            if media_instance:
                self._delete_file(media_instance)
